...
import { Clock, MapPin, Users, Briefcase, FileText, ExternalLink } from "lucide-react";
  ...
  const fallbackVacancies = [
    {
      ...
      description: "Проведение научных исследований в области компьютерного зрения и нейросетевых технологий, анализ современных тенденций, методов и алгоритмов, оптимизация существующих алгоритмов для повышения их эффективности и точности.",
...
<li>Знание алгоритмов машинного обучения и компьютерного зрения</li>
    ...
    },
    {
      ...
      description: "Проведение научных исследований в области компьютерного зрения и нейросетевых технологий, работа с современными алгоритмами и методами анализа данных.",
      content: `<h3>Основные обязанности:</h3>
<ul>
<li>Сбор, аннотирование и предобработка данных для обучения моделей</li>
<li>Разработка методов аугментации данных для улучшения качества обучения</li>
...
<li>Участие в обсуждениях, семинарах и конференциях для обмена опытом и знаниями</li>
  ...
  const displayVacancies = publishedVacancies.length > 0 ? publishedVacancies : fallbackVacancies as any[];
  ...
  const extractFirstSentence = (content: string, maxLength: number = 150) => {
    ...
    while (i < sentences.length && (result + sentences[i]).length <= maxLength) {
      result += '.' + sentences[i];
      i++;
    }
  ...
  return (
    <section id="vacancies" className="py-16 bg-gradient-to-b from-background via-background to-[#f8fafc]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center space-y-4 mb-16">
          ...
          <p className="text-xl text-muted-foreground max-w-2xl mx-auto">
            Присоединяйтесь к нашей команде исследователей в области компьютерного зрения и искусственного интеллекта
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-8">
          {displayVacancies.map((vacancy) => {
            ...
            return (
              <Card key={vacancy.id} className="p-6 space-y-6 hover:shadow-lg transition-shadow h-fit">
                <div className="space-y-3">
                  ...
                  <p className="text-muted-foreground text-sm">
                    {(vacancy as any).department || "Лаборатория интеллектуальных систем и анализа данных"}
                  </p>
                ...
                <div className="space-y-3">
                  ...
                  {(vacancy as any).contract && (
                    <div className="flex items-center gap-2 text-sm text-muted-foreground">
                      ...
                      <span className="line-clamp-1">{(vacancy as any).contract}</span>
                    </div>
                  )}
                  <div className="flex items-center gap-2 text-sm text-muted-foreground">
                    <Clock className="h-4 w-4" />
                    <span>
                      Опубликовано {format(new Date(vacancy.createdAt), "d MMMM yyyy", { locale: ru })}
                    </span>
                  </div>
                </div>
